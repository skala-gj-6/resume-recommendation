...
[MVP 사용자 흐름](user-flow.md)의 경험 구조화와 자기소개서 문항별 초안 생성 내부 처리를 정의합니다. 실제 LLM을 붙이지 않는 데모에서는 같은 입출력 계약의 목 AI 구현체를 사용할 수 있습니다.
...
외부 API 계약은 경험 선택 전략을 노출하지 않고, 서버 내부 정책으로 교체할 수 있게 합니다.

```text
ExperienceSelectionPolicy
├─ AllExperiencesPolicy       모든 저장 경험을 생성 호출에 전달
└─ TwoStageSelectionPolicy    요약으로 1~3개 선택 후 본문 생성
```
...
| 방식 | 장점 | 주의점 |
|---|---|---|
| 모든 경험 전달 | 구현이 단순하고 경험이 적을 때 문맥 손실이 적음 | 경험이 많으면 토큰 비용과 선택 일관성이 나빠질 수 있음 |
| 2단계 선택 | 많은 경험에서도 입력을 제한하고 선택 근거를 분리 가능 | 정상 기준 LLM 호출이 2회가 되고 구현이 복잡함 |

사용자 경험이 소수인 데모라면 모든 경험 전달로 시작할 수 있습니다. 선택한 정책과 무관하게 실제 사용한 경험만 `DRAFT_EXPERIENCE`에 저장합니다.

현재 목 생성기는 교체 가능성을 확인하기 위한 임시 구현으로 전체 경험 후보를 받은 뒤 최근 경험부터 최대 2건을 선택합니다. 이는 최종 경험 선택 정책이 아니며 실제 LLM 연결 시 이 절의 합의 결과로 교체합니다.
...
완전한 LLM 재현을 목표로 하지는 않습니다. 전체 후보 목록, 프롬프트 원문, 모델 버전의 저장 여부는 구현 전 합의 항목입니다.
