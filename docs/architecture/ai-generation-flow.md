...
[MVP 사용자 흐름](user-flow.md)의 경험 구조화와 자기소개서 초안 생성 내부 처리를 정의합니다. 실제 LLM을 붙이지 않는 데모에서는 같은 입출력 계약을 구현한 목 AI 어댑터를 사용할 수 있습니다.
...
빠른 경험 저장과 백그라운드 구조화는 제공하지 않습니다. 사용자가 구조화 결과를 확인한 뒤 저장 API를 호출합니다.
