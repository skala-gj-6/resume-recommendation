...
- Spring에는 Mock 제공자의 공고 상세·추천 후보를 호출하는 클라이언트가 구현되어 있습니다.
- 사용자 경험 조회, 추천 결과 저장, 공개 REST 엔드포인트는 API 명세에 정의된 다음 구현 범위입니다.

## 현재 구현 범위

| 구분 | 현재 상태 |
|---|---|
| 공고 목록·상세·목 추천 후보 API | `MOCK`에서 실행 가능 |
| 기업·기업정보 PostgreSQL 초기 적재 | Spring 시작 시 실행 가능 |
| Spring → Mock 제공자 클라이언트 | 구현 및 단위 테스트 완료 |
| 로그인·경험·추천 저장·지원서·AI API | 명세만 존재하며 아직 미구현 |
...
- Compose의 `depends_on`은 최초 준비 순서만 관리합니다. Spring 클라이언트에는 연결·읽기 타임아웃과 타입별 내부 예외가 있지만, 공개 API의 `502` 응답 변환은 공개 REST 엔드포인트를 구현할 때 함께 추가해야 합니다.
