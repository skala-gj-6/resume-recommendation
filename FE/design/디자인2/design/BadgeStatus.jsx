import { IconNormalDot } from './IconNormalDot.jsx';
import { RatioVertical } from './RatioVertical.jsx';

// figma node: 16215:43108 _Badge/Status (2 variants)
const __venc = (v) => String(v).replace(/[%|=]/g, encodeURIComponent);
const __vkey = (p) => "available=" + __venc(p.available);

export function BadgeStatus(_p = {}) {
  const props = { ..._p, updated: _p.updated ?? false, text: _p.text ?? "platform", available: _p.available ?? false };
  const __body0 = () => (
    <div className={props.className} style={{
      width: "fit-content",
      borderRadius: 4,
      backgroundColor: "var(--interaction-inactive)",
      display: "flex",
      flexDirection: "row",
      gap: 3,
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            }}>{props.icon2 ?? <IconNormalDot name={"dot"} />}</div>
        </div>
      </div>
      )}
    </div>
  ...
  const __impls = {
    ...
    "available=false": __body0,
    // figma: Available=True
    "available=true": __body1,
  };
